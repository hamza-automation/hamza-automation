<div align="center">
# Hey 👋 I'am Hamza
### 🐍 Python Automation & Web Scraping Developer
I build **web scraping, data mining, browser automation, and AI-powered automation systems**.
I enjoy turning repetitive manual work into reliable automated workflows ⚙️🤖
<br>
[![GitHub](https://img.shields.io/badge/GitHub-Profile-181717?style=for-the-badge&logo=github)](https://github.com/)
[![Python](https://img.shields.io/badge/Python-Developer-3776AB?style=for-the-badge&logo=python&logoColor=white)](https://www.python.org/)
</div>
---
## 👨‍💻 About Me
I'm a Python developer focused on **automation and data extraction**.
I build systems that can:
- 🌐 Scrape websites and collect structured data
- 🤖 Automate browser interactions
- 📊 Clean, validate, and organize datasets
- 🔄 Handle pagination and large data collections
- 🧹 Remove duplicate and incomplete records
- ⚙️ Build reusable automation tools
- 🗂️ Export clean data to CSV
- 🕐 Run automated jobs on schedules
- 🧠 Integrate AI/LLMs into automation workflows
My goal is simple:
> **Use code and automation to turn repetitive work into useful systems. 🚀**
---
## 🛠️ Technologies & Tools
### 🐍 Python
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
### 🌐 Web Scraping & Browser Automation
![Playwright](https://img.shields.io/badge/Playwright-2EAD33?style=for-the-badge&logo=playwright&logoColor=white)
![Selenium](https://img.shields.io/badge/Selenium-43B02A?style=for-the-badge&logo=selenium&logoColor=white)
![BeautifulSoup](https://img.shields.io/badge/BeautifulSoup-000000?style=for-the-badge)
### ⚙️ Automation & Data
![Pandas](https://img.shields.io/badge/Pandas-150458?style=for-the-badge&logo=pandas&logoColor=white)
![Requests](https://img.shields.io/badge/Requests-000000?style=for-the-badge)
![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white)
### 🤖 AI & Intelligent Automation
- 🧠 LLM APIs
- 🤖 AI Agents
- 🔧 Tool Calling
- 🌐 AI + Browser Automation
- ⚡ AI-assisted development
---
## 🚀 Featured Projects
### 🛒 E-commerce Product Scraper
A Python-based web scraping system designed to collect and structure product information from websites.
**Features:**
- 🔎 Product discovery
- 📄 Pagination handling
- 💰 Price extraction
- 📦 Product information extraction
- 🧹 Data cleaning
- ♻️ Duplicate removal
- 📊 CSV export
- 📝 Logging and error handling
---
### 🏠 Real Estate Agent Data Mining
An automated browser-based system for collecting and structuring publicly available real estate agent information.
**Collected information includes:**
| Field | Data |
|---|---|
| 👤 | Agent |
| 🏢 | Agency |
| 📈 | Sales 12M |
| 💰 | Total Sales |
| 🏷️ | Price Range |
| 💵 | Avg. Price |
| ⏳ | Exp. (Years) |
| 📞 | Phone |
| 📧 | Email |
| 📍 | Location |
| 🔗 | Profile |
---
## 🧩 What I Like Building
```text
🌐 Web Scrapers
        ↓
🧹 Data Cleaning
        ↓
📊 Structured Data
        ↓
⚙️ Automation
        ↓
🤖 AI / Intelligent Workflows
        ↓
🚀 Useful Systems


🎯 My Focus
    Build useful automation systems, solve real problems, and keep improving every day.

Thanks for stopping by!👋
Lets build something useful.⚡
